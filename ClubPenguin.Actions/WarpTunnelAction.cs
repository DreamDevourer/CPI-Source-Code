...
public class WarpTunnelAction : ClubPenguin.Actions.Action
{
	...
	private void calculateSplines()
	{
		...
		Transform[] componentsInChildren = Waypoints.GetComponentsInChildren<Transform>(includeInactive: true);
	...
	protected override void OnDisable()
	{
		...
		if (FakeVehicle != null)
		{
			FakeVehicle.SetActive(value: true);
		}
		if (base.gameObject.CompareTag("Player"))
		{
			GameObject gameObject = GameObject.FindWithTag(UIConstants.Tags.UI_Tray_Root);
			...
			if (Service.Get<QuestService>().ActiveQuest != null)
			{
				Service.Get<EventDispatcher>().DispatchEvent(new HudEvents.ShowHideQuestNotifier(show: true));
			}
			else
			{
