...
public class HC128Engine : IStreamCipher
{
	...
	private unsafe uint Step()
	{
		...
		if (cnt < 512)
		{
			uint[] array;
			IntPtr intPtr;
			(array = p)[(long)(intPtr = (IntPtr)(void*)num)] = array[(long)intPtr] + G1(p[Dim(num, 3u)], p[Dim(num, 10u)], p[Dim(num, 511u)]);
		...
		else
		{
			uint[] array;
			IntPtr intPtr;
			(array = q)[(long)(intPtr = (IntPtr)(void*)num)] = array[(long)intPtr] + G2(q[Dim(num, 3u)], q[Dim(num, 10u)], q[Dim(num, 511u)]);
	...
	private void Init()
	{
		...
		for (int i = 0; i < 16; i++)
		{
			...
			array3[num] = (uint)((int)array2[(long)intPtr] | (key[i] << 8 * (i & 3)));
		}
		...
		for (int j = 0; j < iv.Length && j < 16; j++)
		{
			...
			array4[num2] = (uint)((int)array2[(long)intPtr] | (iv[j] << 8 * (j & 3)));
		}
		Array.Copy(array, 8, array, 12, 4);
		for (uint num3 = 16u; num3 < 1280; num3++)
		{
			array[num3] = F2(array[num3 - 2]) + array[num3 - 7] + F1(array[num3 - 15]) + array[num3 - 16] + num3;
		}
