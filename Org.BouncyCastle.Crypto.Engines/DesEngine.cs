...
public class DesEngine : IBlockCipher
{
	...
	protected static int[] GenerateWorkingKey(bool encrypting, byte[] key)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			...
			for (int m = 0; m < 24; m++)
			{
				if (array3[pc2[m]])
				{
					...
					array5[num5] = (array4[(long)intPtr] | bigbyte[m]);
				}
				if (array3[pc2[m + 24]])
				{
					int[] array4;
					...
					array6[num6] = (array4[(long)intPtr] | bigbyte[m]);
				}
			}
		}
		for (int n = 0; n != 32; n += 2)
		{
			int num7 = array[n];
			int num8 = array[n + 1];
			array[n] = (((num7 & 0xFC0000) << 6) | ((num7 & 0xFC0) << 10) | (int)((uint)(num8 & 0xFC0000) >> 10) | (int)((uint)(num8 & 0xFC0) >> 6));
			array[n + 1] = (((num7 & 0x3F000) << 12) | ((num7 & 0x3F) << 16) | (int)((uint)(num8 & 0x3F000) >> 4) | (num8 & 0x3F));
		}
		return array;
	}
