...
public class WebViewPurchaseController
{
	...
	public void ReloadManageAccountFlow(CSGConfig csgConfig)
	{
		...
		{
			IContentInterruption gate = new ParentGate();
			webViewController.Show(text, gate, "Membership.ManageAccount.CSG.WebviewerTitle", AllowPopups: true, openPopupInNewBrowser: false, 1.8f, 3f);
		}
		else
		{
	...
	private void sendBI(JsonData biContent)
	{
		...
		string text = (string)biContent["bi-method"];
		if (text == null)
		{
			return;
		}
