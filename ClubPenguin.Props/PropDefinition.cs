...
public class PropDefinition : StaticGameDataDefinition, IMemberLocked
{
	...
	public void OnValidate()
	{
		...
		for (int i = 0; i < num; i++)
		{
			if (Tags[i] == null)
			{
				Log.LogError(this, $"O_o\t Found NULL tag in element {i} of Prop {Name}, Id {Id}");
				continue;
			}
			...
			if (hashSet.Contains(tag))
			{
				if (Tags[i].name != tag)
				{
					Log.LogError(this, $"O_o\t Found bad tag in element {i} of Prop {Name}, Id {Id}. It's name field '{Tags[i].name}' doesn't match it's tag field '{Tags[i].Tag}'");
				}
				else
				{
					Log.LogError(this, $"O_o\t Found duplicate tag '{tag}' in element {i} of Prop {Name}, Id {Id}");
				}
			}
			else
			{
