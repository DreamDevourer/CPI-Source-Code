...
public class BigInteger
{
	...
	public BigInteger(byte[] bytes, int offset, int length)
	{
		...
		if ((sbyte)bytes[offset] < 0)
		{
			...
			array3[num6] = (byte)(array2[(long)intPtr] + 1);
	...
	public BigInteger(int bitLength, int certainty, Random random)
	{
		...
		while (true)
		{
			...
			array3[num2] = (byte)(array2[(long)intPtr] | 1);
			magnitude = MakeMagnitude(array, 0, array.Length);
			nBits = -1;
			mQuote = 0;
			...
			for (int i = 1; i < magnitude.Length - 1; i++)
			{
				...
				int num3 = i;
				intPtr = (IntPtr)num3;
				array5[num3] = (array4[(long)intPtr] ^ random.Next());
	...
	private static int[] AddMagnitudes(int[] a, int[] b)
	{
		...
		if (num3 != 0)
		{
			while (num >= 0)
			{
				...
				if ((array2[num7] = array[(long)intPtr] + 1) != 0)
				{
					break;
				}
			}
	...
	private static int[] Square(int[] w, int[] x)
	{
		...
		if (--num >= 0)
		{
			...
			int num7 = num;
			IntPtr intPtr = (IntPtr)num7;
			array2[num7] = array[(long)intPtr] + (int)(num4 >> 32);
		}
		return w;
	}
	...
	private int[] LastNBits(int n)
	{
		...
		int val = (n + 32 - 1) / 32;
		val = Math.Min(val, magnitude.Length);
		...
		else
		{
			...
			for (int num6 = num - 1; num6 >= 0; num6--)
			{
				...
				if (num5 >= 0)
				{
					...
					array3[num8] = (array2[(long)intPtr] | (magnitude[num5] << num4));
				}
			}
		}
		return new BigInteger(sign, array, checkMag: false);
	...
	private static int[] Subtract(int xStart, int[] x, int yStart, int[] y)
	{
		...
		if (num3 != 0)
		{
			...
			while ((array2[num5] = array[(long)intPtr] - 1) == -1);
		}
		return x;
	}
	...
	private BigInteger FlipExistingBit(int n)
	{
		...
		array3[num] = (array2[(long)intPtr] ^ (1 << (n & 0x1F)));
		return new BigInteger(sign, array, checkMag: false);
	}
}
