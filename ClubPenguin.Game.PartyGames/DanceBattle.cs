...
public class DanceBattle : AbstractPartyGameSession
{
	...
	{
		changeCameraController(evt.DanceBattleVisualsController.ScreenCamera, evt.DanceBattleVisualsController.ScreenCameraTarget, hideControls: true);
		return false;
	}
	...
	{
		changeControlState(ControlsState.Enabled);
		changeCameraController(evt.DanceBattleVisualsController.DanceCamera, evt.DanceBattleVisualsController.DanceCameraTarget, hideControls: false);
		return false;
	}
	...
	private bool onDanceBattleStartEvent(DanceBattleEvents.DanceBattleStart evt)
	{
		...
		danceBattleVisualsController.setPlayerIsInGame(isPlayerInGame: true);
		changeCameraController(evt.DanceBattleVisualsController.ScreenCamera, evt.DanceBattleVisualsController.ScreenCameraTarget, hideControls: true);
		CoroutineRunner.Start(changeCameraControllerWithDelay(7f, evt.DanceBattleVisualsController.DanceCamera, evt.DanceBattleVisualsController.DanceCameraTarget, hideControls: false), this, "ChangeDanceBattleCamera");
		return false;
	}
	...
	private void changeToDefaultCamera()
	{
		...
		evt.Controller = currentCameraController;
		dispatcher.DispatchEvent(evt);
		dispatcher.DispatchEvent(new CinematographyEvents.ChangeCameraTarget(ClubPenguin.SceneRefs.ZoneLocalPlayerManager.LocalPlayerGameObject.transform));
	}
	...
	private bool onFinalResultsShown(DanceBattleEvents.FinalResultsShown evt)
	{
		if (!evt.IsTie)
		{
			...
			ClubPenguin.SceneRefs.CelebrationRunner.PlayCelebrationAnimation(list);
		}
		return false;
	}
