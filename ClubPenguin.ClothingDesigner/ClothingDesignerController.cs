...
public class ClothingDesignerController
{
	...
	private bool onStateChangeInventory(ClothingDesignerUIEvents.ChangeStateInventory evt)
	{
		...
		else
		{
			...
			ClothingDesignerContext.EventBus.DispatchEvent(new ClothingDesignerUIEvents.UpdateCameraState(ClothingDesignerCameraState.Inventory, animateCamera: true));
		}
		return false;
	}
	...
	private bool onStateChangeCustomizer(ClothingDesignerUIEvents.ChangeStateCustomizer evt)
	{
		...
		{
			ClothingDesignerContext.EventBus.DispatchEvent(new ClothingDesignerUIEvents.UpdateCameraState(ClothingDesignerCameraState.CatalogCustomizer, animateCamera: false));
			showCatalogCustomizer = false;
		}
		else
		{
			ClothingDesignerContext.EventBus.DispatchEvent(new ClothingDesignerUIEvents.UpdateCameraState(ClothingDesignerCameraState.Customizer, animateCamera: true));
		}
		return false;
	}
	...
	private bool onHideCatalog(ClothingDesignerUIEvents.HideCatalog evt)
	{
		...
		ClothingDesignerContext.EventBus.DispatchEvent(new ClothingDesignerUIEvents.UpdateCameraState(ClothingDesignerCameraState.Inventory, animateCamera: false));
	...
	{
		dependencies.ScreenContent.SetActive(value: true);
		dependencies.CatalogContainer.SetActive(value: false);
		penguinLighting.SetActive(value: true);
	}
