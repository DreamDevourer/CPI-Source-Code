...
public class TubeLoader : MonoBehaviour
{
	...
	{
		if (tubeDefinitions.TryGetValue(tubeId, out TubeDefinition definition))
		{
			AssetRequest<GameObject> request = Content.LoadAsync(definition.TubeAssetContentKey);
			yield return request;
			yield return new WaitForSeconds(0.25f);
			removeCurrentTube();
			GameObject tubeObject = Object.Instantiate(request.Asset, TubeParent, worldPositionStays: false);
