...
public class Chomper : ProximityBroadcaster
{
	...
	{
		base.Start();
		penguinObj = ClubPenguin.SceneRefs.ZoneLocalPlayerManager.LocalPlayerGameObject;
	}
	...
	private void enableUIControls(bool isEnabled)
	{
		...
		else
		{
			...
			Service.Get<EventDispatcher>().DispatchEvent(new UIDisablerEvents.DisableUIElement("DailyChallengesTab", hideElement: true));
		}
	}
