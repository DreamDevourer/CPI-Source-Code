...
public class PlayerCardController : MonoBehaviour
{
	...
	private void setUpPresenceData(PresenceData presenceData)
	{
		...
		if (zoneDefinition != null)
		{
			...
			flag = true;
			bool flag2 = false;
			if (dataEntityCollection.TryGetComponent(dataEntityCollection.LocalPlayerHandle, out PresenceData component))
			{
	...
	private void setUpProfileData(ProfileData profileData)
	{
		...
		MembershipSpriteSelector.gameObject.SetActive(value: true);
		OnlineSpriteSelector.gameObject.SetActive(value: true);
	...
	private bool onFriendLocationReceived(FriendsServiceEvents.FriendLocationInRoomReceived evt)
	{
		...
		if (!(Vector3.Distance(ClubPenguin.SceneRefs.ZoneLocalPlayerManager.LocalPlayerGameObject.transform.position, location) <= 3f))
		{
			LocomotionTracker component = ClubPenguin.SceneRefs.ZoneLocalPlayerManager.LocalPlayerGameObject.GetComponent<LocomotionTracker>();
			...
			else if (component.IsCurrentControllerOfType<SwimController>() && dataEntityCollection.TryGetComponent(Handle, out component2))
			{
				...
				{
					Animator component4 = ClubPenguin.SceneRefs.ZoneLocalPlayerManager.LocalPlayerGameObject.GetComponent<Animator>();
			...
			ClubPenguin.SceneRefs.ZoneLocalPlayerManager.LocalPlayerGameObject.transform.position = location;
