...
public class UIDisabler : MonoBehaviour
{
	...
	private void disableQuestHud()
	{
		...
		if (gameObject != null)
		{
			QuestHud componentInChildren = gameObject.GetComponentInChildren<QuestHud>(includeInactive: true);
	...
	{
		if (ClubPenguin.SceneRefs.UiTrayRoot != null)
		{
			if (instant)
			{
				ClubPenguin.SceneRefs.UiTrayRoot.GetComponent<StateMachineContext>().SendEvent(new ExternalEvent("Root", "noUIInstant"));
			}
			else
			{
				ClubPenguin.SceneRefs.UiTrayRoot.GetComponent<StateMachineContext>().SendEvent(new ExternalEvent("Root", "noUI"));
			}
		}
	}
	...
	{
		if (ClubPenguin.SceneRefs.UiTrayRoot != null && ClubPenguin.SceneRefs.UiTrayRoot.GetComponent<StateMachineContext>() != null)
		{
			ClubPenguin.SceneRefs.UiTrayRoot.GetComponent<StateMachineContext>().SendEvent(new ExternalEvent("Root", "restoreUI"));
		}
	}

	private void disableChatUI()
	{
		GameObject uiChatRoot = ClubPenguin.SceneRefs.UiChatRoot;
		if (!(uiChatRoot != null))
		{
			return;
		}
	...
	private void restoreCamera()
	{
		...
		Service.Get<EventDispatcher>().DispatchEvent(new UIDisablerEvents.CameraCullingStateChanged(isRenderingEnabled: true));
	}
}
