...
public class FriendsGridController : AbstractPlayerListController
{
	...
	protected override void start()
	{
		...
		dataEntityCollection.EventDispatcher.AddListener((EventHandlerDelegate<DataEntityEvents.ComponentAddedEvent<MembershipData>>)((AbstractPlayerListController)this).onMembershipDataAdded, EventDispatcher.Priority.DEFAULT);
	...
	private List<string> getOutgoingInvitationDisplayNames()
	{
		...
		{
			if (dataEntityCollection.TryGetComponent(outgoingInvitationsList[i], out DisplayNameData component) && !string.IsNullOrEmpty(component.DisplayName))
			{
	...
	private List<DataEntityHandle> getMatchingFriends(string substring, List<DataEntityHandle> friendsList)
	{
		...
		{
			if (dataEntityCollection.TryGetComponent(friendsList[i], out DisplayNameData component) && component != null && !string.IsNullOrEmpty(component.DisplayName))
			{
				list.Add(component.DisplayName);
	...
	protected override DataEntityHandle getHandleFromPoolIndex(int poolIndex)
	{
		...
		throw new IndexOutOfRangeException($"The index is beyond the combined ranges of the friends and outgoing invitations lists. poolIndex = {poolIndex}, List Sizes: friends = {visiblePlayersList.Count}, invitations = {visibleOutgoingInvitationsList.Count}, list index = {listIndex}.");
	}
