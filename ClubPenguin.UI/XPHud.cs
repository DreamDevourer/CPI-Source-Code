...
public class XPHud : MonoBehaviour
{
	...
	public XPHudState State => state;

	public string CurrentMascotName => currentMascotName;
	...
	private void disableUI()
	{
		...
		ClubPenguin.SceneRefs.UiTrayRoot.GetComponent<StateMachineContext>().SendEvent(new ExternalEvent("Root", "minNPC"));
	...
	{
		currentXpDisplay.XPAnimator.SetBool("isShown", value: true);
		if (state == XPHudState.closed && this.HudOpened != null)
		{
			this.HudOpened(base.gameObject);
		}
		GetComponentInChildren<MascotLevelDisplay>().SetUpMascotLevel(isLocalPlayer: true, null, currentMascotLevel);
	}
