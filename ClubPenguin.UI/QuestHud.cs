...
public class QuestHud : MonoBehaviour
{
	...
	{
		CoroutineRunner.StopAllForOwner(this);
		hideCommunicatorHud(hidePhone: false);
		hideMessageHud();
		toggleCommunicatorHudEnabled(enabled: false);
	...
	{
		toggleCommunicatorHudEnabled(enabled: false);
		QuestCommunicatorHud.SetOpen(open: false);
	}
	...
	private IEnumerator playTextUpdatedAnimation()
	{
		...
		{
			enableAnimator();
			animator.SetBool("isTextUpdated", value: true);
			yield return null;
			animator.SetBool("isTextUpdated", value: false);
		}
	}
	...
	private void showCommunicatorHud()
	{
		...
		else if (!QuestCommunicatorHud.IsSuppressed && !QuestCommunicatorHud.IsPermanentlySuppressed)
		{
			...
			QuestCommunicatorHud.SetOpen(open: true);
			toggleCommunicatorHudEnabled(enabled: true);
			if (!animator.GetBool("isCommunicatorOpen"))
			{
				enableAnimator();
				animator.SetBool("isCommunicatorOpen", value: true);
			}
			showSubtasks(forceOpen: true);
			Service.Get<EventDispatcher>().DispatchEvent(default(HudEvents.ShowCellPhoneHud));
			Service.Get<EventDispatcher>().DispatchEvent(new UIDisablerEvents.EnableUIElement("CellphoneButton"));
		}
	}
	...
	private void hideCommunicatorHud(bool hidePhone = true)
	{
		...
		animator.SetBool("isCommunicatorOpen", value: false);
	}
	...
	private void showMessageHud()
	{
		...
		enableAnimator();
		toggleMessageHudEnabled(enabled: true);
		if (!animator.GetBool("isMessageOpen"))
		{
			CoroutineRunner.Start(disableCommunicatorPanel(), this, "");
			animator.SetBool("isMessageOpen", value: true);
			QuestPointerPanel.SetActive(value: true);
		}
	...
	{
		yield return new WaitForEndOfFrame();
		CommunicatorPanel.SetActive(value: false);
	}

	private IEnumerator enableCommunicatorPanel()
	{
		yield return new WaitForEndOfFrame();
		CommunicatorPanel.SetActive(value: true);
	}
	...
	private void hideMessageHud()
	{
		...
		if (animator.GetBool("isMessageOpen"))
		{
			animator.SetBool("isMessageOpen", value: false);
		}
	...
	private void showSubtasks(bool forceOpen = false)
	{
		if (QuestCommunicatorHud.SubtaskCount > 0)
		{
			...
			{
				enableAnimator();
				animator.SetBool("isSubtasksOpen", value: true);
		...
		{
			enableAnimator();
			animator.SetBool("isSubtasksOpen", value: false);
		}
	}
