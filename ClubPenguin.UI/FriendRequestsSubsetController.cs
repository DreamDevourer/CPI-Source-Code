// FriendRequestsSubsetController
using ClubPenguin;
...
public class FriendRequestsSubsetController : MonoBehaviour
{
	...
	public void Show()
	{
		bool active = incomingFriendInvitationsList.Count > 0;
		FriendsRequestsHeader.SetActive(active);
		FriendRequestsContainer.gameObject.SetActive(value: true);
	}

	public void Hide()
	{
		FriendsRequestsHeader.SetActive(value: false);
		FriendRequestsContainer.gameObject.SetActive(value: false);
	}
	...
	{
		showFriendRequests(isSettingUp: true, friendRequestItemPrefab);
	}
	...
	private void updateIncomingInvitationList(List<DataEntityHandle> updatedFriendInvitations)
	{
		...
		showFriendRequests(isSettingUp: false);
		TitleText.text = string.Format(screenTitle, count);
	}

	private MembershipType getMembershipType(DataEntityHandle handle)
	{
		return dataEntityCollection.GetComponent<MembershipData>(handle)?.MembershipType ?? MembershipType.None;
	}
