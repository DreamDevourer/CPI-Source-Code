...
public class StatusIndicatorController : MonoBehaviour
{
	...
	{
		if (dataEntityCollection.TryGetComponent(evt.Handle, out SessionIdData _))
		{
	...
	private void onStatusChanged(PlayerStatusData playerStatusData, string questMascotName)
	{
		...
		if (dataEntityCollection.TryGetComponent(entityByComponent, out SessionIdData sessionId))
		{
			if (mascotNameToIconMap.TryGetValue(questMascotName, out Sprite value))
			{
	...
	private bool onComponentRemoved(DataEntityEvents.ComponentRemovedEvent evt)
	{
		...
		if (playerStatusData != null && dataEntityCollection.TryGetComponent(evt.Handle, out SessionIdData component))
		{
	...
	{
		if (dataEntityCollection.TryFindEntity<SessionIdData, long>(sessionId, out DataEntityHandle dataEntityHandle) && dataEntityCollection.TryGetComponent(dataEntityHandle, out PlayerStatusData component))
		{
