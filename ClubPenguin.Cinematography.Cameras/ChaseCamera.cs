...
public class ChaseCamera : MonoBehaviour
{
	...
	{
		bool result = false;
		if (Physics.Raycast(base.transform.position, feelDir, out RaycastHit hitInfo, rayDist, raycastLayerMask))
		{
			result = true;
	...
	private void avoidWalls()
	{
		...
		{
			if (feel(feeler[i], CollisionRadius, out Vector3 collisionPoint, out Vector3 collisionNormal, out float _))
			{
				if (i == 0)
				{
					break;
