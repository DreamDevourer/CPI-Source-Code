...
public class SwitchComponent : Fabric.Component, GlobalSwitch.IListener, IRTPPropertyListener
{
	[Serializable]
	public class GlobalSwitchContainer
	{
		...
		[SerializeField]
		public List<Fabric.Component> _components = new List<Fabric.Component>();
	}

	[SerializeField]
	[HideInInspector]
	public Fabric.Component _selectedComponent;

	private Fabric.Component _defferedSelectedComponent;
	...
	private void DoSwitch(double time)
	{
		if (_defferedSelectedComponent != null)
		{
			if (_selectedComponent != null)
			{
				_selectedComponent.StopInternal(stopInstances: false, forceStop: false, 0f, 0.5f, time);
			}
	...
	bool IRTPPropertyListener.UpdateProperty(RTPProperty property, float value, RTPPropertyType type)
	{
		...
		if (property._property == 8 && property._name == "Switch" && _components.Count > 0)
		{
			...
			Fabric.Component component = _components[index];
