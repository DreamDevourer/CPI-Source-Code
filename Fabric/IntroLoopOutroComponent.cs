// IntroLoopOutroComponent
using Fabric;
using System.Collections;
using UnityEngine;

[AddComponentMenu("Fabric/Components/IntroLoopOutroComponent")]
public class IntroLoopOutroComponent : Fabric.Component
{
	public enum Stage
	{
		Intro,
	...
	[HideInInspector]
	[SerializeField]
	public Fabric.Component[] _stages = new Fabric.Component[3];
	...
	private IEnumerator onPlayLoop(float time)
	{
		...
		if (_isCoroutineActive)
		{
			...
			{
				_stages[0].StopInternal(stopInstances: false, forceStop: false, 0f, 0.5f);
			}
			_stages[1].PlayInternal(_componentInstance, 0f, 0.5f);
		}
	...
	public override void StopInternal(bool stopInstances, bool forceStop, float target, float curve, double scheduleEnd = 0.0)
	{
		...
		else if (_activeStage == Stage.Loop && !_playLoopToEnd)
		{
			if (_stages[0] != null)
			{
				_stages[0].StopInternal(stopInstances: false, forceStop: false, 0f, 0.5f, scheduleEnd);
			}
			if (_stages[1] != null)
			{
				_stages[1].StopInternal(stopInstances: false, forceStop: false, 0f, 0.5f, scheduleEnd);
				_isCoroutineActive = false;
			}
	...
	internal override void OnFinishPlaying(double time)
	{
		...
		{
			_stages[1].StopInternal(stopInstances: false, forceStop: false, 0f, 0.5f, time);
