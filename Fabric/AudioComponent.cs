...
public class AudioComponent : Fabric.Component
{
	...
	public GameObject AudioSourceGameObject => _audioSourceGameObject;

	public AudioComponentState CurrentState => _currentState;

	public DynamicLoadAsyncState LoadAsyncState => _dynamicLoadAsyncState;
	...
	public AudioSource[] LoopRegions => _loopRegionAudioSources;
	...
	public double DSPTimeTriggered => _dspTimeTriggered;
	...
	public void InitializeAudioComponent(bool inPreviewMode = false, AudioSource audioSource = null)
	{
		...
		else
		{
			...
			if (_audioSource != null)
			{
				...
				Generic.SetGameObjectActive(_audioSourceGameObject, active: false);
				if (_loopRegionAudioSources != null && _loopRegionAudioSources.Length > 0 && _loopRegionAudioSources[1] != null)
				{
					Generic.SetGameObjectActive(_loopRegionAudioSources[1].gameObject, active: false);
				}
				if (inPreviewMode)
				{
	...
	public override void Destroy()
	{
		if (!_quitting)
		{
			StopAudioComponent(notifyParent: false);
			_audioClip = null;
			base.Destroy();
		}
	}
	...
	private IEnumerator DynamicAudioClipLoad(AudioClipHandle audioClipHandle)
	{
		...
		if (audioClipHandle.RefCount == 0)
		{
			...
			audioClipHandle.AudioClip = WWWAudioExtensions.GetAudioClip(www);
		}
		_dynamicLoadAsyncState = DynamicLoadAsyncState.Loaded;
		audioClipHandle.IncRef(loadAudioClip: false);
	...
	public override void PlayInternal(ComponentInstance zComponentInstance, float target, float curve, bool dontPlayComponents)
	{
		...
		{
			Generic.SetGameObjectActive(_audioSourceGameObject, active: true);
			if (_loopRegionAudioSources != null && _loopRegionAudioSources.Length > 0 && _loopRegionAudioSources[1] != null)
			{
				Generic.SetGameObjectActive(_loopRegionAudioSources[1].gameObject, active: true);
			}
		}
			...
			if (_markers != null && _markers.Count > 0)
			{
				...
				for (int i = 0; i < _markers.Count; i++)
				{
					...
					if (OnMarker(num11))
					{
						...
						{
							StopInternal(stopInstances: false, forceStop: false, 0f, 0.5f, num11);
						}
					}
					if (HasValidEventNotifier())
					{
	...
	public override void StopInternal(bool stopInstances, bool forceStop, float target, float curve, double scheduleEnd = 0.0)
	{
		...
		if (forceStop)
		{
			...
			{
				RemoveDSPInstances();
				FabricManager.Instance.AudioSourcePoolManager.Free(_audioSource, callStop: true);
				if (_loopRegionAudioSources != null && _loopRegionAudioSources.Length > 0 && _loopRegionAudioSources[1] != null)
				{
					FabricManager.Instance.AudioSourcePoolManager.Free(_loopRegionAudioSources[1]);
				}
