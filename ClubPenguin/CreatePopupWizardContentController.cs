// CreatePopupWizardContentController
using ClubPenguin;
...
public class CreatePopupWizardContentController : CreatePopupContentController
{
	...
	private void shardSetup()
	{
		...
		if (shard != -1)
		{
			Service.Get<ICPSwrveService>().TestImpression(swrveResourceUniqueID, shard.ToString(), applied: true);
		}
		if (shard == -1)
		{
			shard = 0;
		...
		{
			shardInitialActiveFormContainers[shard].SetActive(value: true);
		...
		if (shardTransitionStateHandlers != null && shardTransitionStateHandlers.Length > 0 && shardTransitionStateHandlers[shard] != null)
		{
			...
			{
				shardTransitionStateHandlers[i].SetActive(value: false);
			}
		}
