...
public class ValidationInputController : MonoBehaviour
{
	...
	private void Update()
	{
		if (validator.TextInput.isFocused)
		{
			...
			{
				DescriptionBox.SetActive(value: true);
			}
		...
		{
			DescriptionBox.SetActive(value: false);
		}
	...
	private string prepareSuggestionButtons(string errorMessage)
	{
		...
		{
			if (item.gameObject.GetComponentInChildren<Button>(includeInactive: true) != null)
			{
	...
	private IEnumerator loadButtonFromPrefab(string suggestedName)
	{
		...
		suggestionGameObject.transform.SetParent(ErrorBox.transform, worldPositionStays: false);
		yield return null;
	}
