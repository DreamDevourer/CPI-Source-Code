...
public class OfflineDefinitionLoader : IOfflineDefinitionLoader
{
	...
	public Reward GetClaimableReward(int rewardId)
	{
		...
		if (dictionary.TryGetValue(rewardId, out ClaimableRewardDefinition value) && value.Reward != null)
		{
	...
	public void SubtractEquipmentCost(int templateDefinitionId)
	{
		...
		if (dictionary.TryGetValue(templateDefinitionId, out TemplateDefinition value))
		{
	...
	public int GetSpinResult(Reward spinReward, Reward chestReward)
	{
		...
		if (spinReward.TryGetValue(out MascotXPReward rewardable))
		{
			bool flag2 = false;
			ProgressionService progressionService = Service.Get<ProgressionService>();
	...
	private static CellPhoneDailySpinActivityDefinition.ChestDefinition? getChestDefinitionForId(int chestId, CellPhoneDailySpinActivityDefinition dailySpinDefinition)
	{
		...
		{
			CellPhoneDailySpinActivityDefinition.ChestDefinition current = chestDefinition;
			if (current.ChestId == chestId)
			{
				return current;
			}
		}
		...
		foreach (LootTableRewardDefinition value in minigameService.Loot.Values)
		{
			if (value.Bucket.Weight != 0)
			{
				if (dictionary2.ContainsKey(value.Bucket.BucketName))
				{
					...
					string bucketName;
					(dictionary3 = dictionary2)[bucketName = value.Bucket.BucketName] = dictionary3[bucketName] + value.Weight;
				}
				else
				{
			...
			if (value.Items != null)
			{
				foreach (DisneyStoreItemDefinition item in value.Items)
				{
					if (item.Id == itemId)
					{
						...
						if (count > 1)
						{
							if (reward.TryGetValue(out ConsumableInstanceReward rewardable))
							{
							...
							if (reward.TryGetValue(out DecorationInstanceReward rewardable2))
							{
							...
							if (reward.TryGetValue(out StructureInstanceReward rewardable3))
							{
	...
	{
		if (dataEntityCollection.TryGetComponent(dataEntityCollection.LocalPlayerHandle, out ProfileData component))
		{
