...
internal class ManageIglooPopupController : MonoBehaviour, IIglooDeleteLayoutErrorHandler, IIglooUpdateDataErrorHandler
{
	...
	{
		NonMemberSlotContainer.gameObject.SetActive(value: false);
		LapsedMembershipNotification.SetActive(value: false);
		for (int i = 0; i < savedIgloosMetaData.SavedIgloos.Count; i++)
		{
	...
	private void init()
	{
		...
		if (savedIgloosMetaData.SavedIgloos.Count == 0)
		{
			privacyButton.Setup(base.transform.parent, savedIgloosMetaData, enableButton: false);
			createButtonCount = 3;
	...
	private void setContainers()
	{
		...
		{
			NonMemberSlotContainer.gameObject.SetActive(value: true);
			MemberSlotContainer.gameObject.SetActive(value: true);
			if (membershipData.HasHadMembership)
			{
				LapsedMembershipNotification.SetActive(value: true);
				igloosCount = 4;
			}
		}
		else
	...
	private void resetSlots()
	{
		...
		NonMemberSlotContainer.gameObject.SetActive(value: false);
		MemberSlotContainer.gameObject.SetActive(value: false);
		LapsedMembershipNotification.SetActive(value: false);
	}
	...
	{
		GameObject gameObject = Object.Instantiate(prefab, slotContainer, worldPositionStays: false);
	...
	public void SetActiveIgloo(long iglooId)
	{
		...
		StartCoroutine(orderIglooCards(animated: true));
	}
	...
	private IEnumerator orderIglooCards(bool animated)
	{
		...
		if (animated)
		{
			...
			inputDisabler.SetActive(value: true);
			for (int i = 0; i < animators.Length; i++)
			{
			...
			inputDisabler.SetActive(value: false);
		}
		else
		{
