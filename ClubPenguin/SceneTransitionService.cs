...
public class SceneTransitionService : MonoBehaviour
{
	...
	public IEnumerator SwapScene(string scene, Dictionary<string, object> sceneArgs = null)
	{
		...
		if (sceneDefinitions.TryGetValue(currentScene, out SceneDefinition sceneDefinition))
		{
		...
		if (intersection.Count == 0)
		{
			...
			yield return loadScene(scene, "Loading", sceneArgs, allowActivation: true);
			yield break;
		}
		...
		if (targetSceneParts.Count > 0)
		{
			...
			yield return loadTargetScene(scene, targetSceneParts, LoadSceneMode.Additive, allowActivation: true);
	...
	{
		unpinCurrentAudioPrefab();
		ClubPenguin.SceneRefs.ClearAll();
	...
	{
		if (!sceneDefinitions.TryGetValue(scene, out SceneDefinition sceneDefinition))
		{
			yield break;
		}
	...
	private IEnumerator loadTargetScene(string sceneName, bool allowActivation)
	{
		...
		if (sceneDefinitions.TryGetValue(sceneName, out SceneDefinition sceneDefinition))
		{
	...
	private IEnumerator loadTargetScene(string sceneName, IEnumerable<string> additiveScenes, LoadSceneMode mainSceneLoadingMode, bool allowActivation)
	{
		...
		if (sceneDefinitions.TryGetValue(sceneName, out SceneDefinition mainSceneDefinition))
		{
		...
		while (assetRequests.Count > 0 || loadableScenes.Count > 0)
		{
			while (loadableScenes.Count > 0)
			{
				...
				if (sceneDefinitions.TryGetValue(additiveScene, out SceneDefinition sceneDefinition) && !(sceneDefinition == null))
				{
					yield return loadPrerequisiteContent(sceneDefinition);
				}
