...
public class CPDataEntityCollectionImpl : DataEntityCollectionDictionaryImpl, CPDataEntityCollection, DataEntityCollection
{
	...
	{
		if (!DataEntityHandle.IsNullValue(LocalPlayerHandle) && TryGetComponent(LocalPlayerHandle, out MembershipData component))
		{
	...
	public IList<Transform> GetRemotePlayers()
	{
		...
		{
			if (TryGetComponent(entitiesByType[i], out GameObjectReferenceData component))
			{
