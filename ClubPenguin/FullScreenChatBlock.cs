...
public class FullScreenChatBlock : MonoBehaviour
{
	...
	private void showActiveChat()
	{
		...
		ActiveTypingPanel.SetActive(value: true);
		showBlockedTextPanel(show: false);
	}
	...
	public void SetIcon(Texture2D icon)
	{
		...
		PenguinIconImage.gameObject.SetActive(value: true);
		if (MembershipSprite != null)
		{
			CPDataEntityCollection cPDataEntityCollection = Service.Get<CPDataEntityCollection>();
			DataEntityHandle handle = findPlayerHandle(SessionId, cPDataEntityCollection);
			if (cPDataEntityCollection.TryGetComponent(handle, out MembershipData component))
			{
	...
	{
		PenguinIconImage.gameObject.SetActive(value: false);
