...
public class CelebrationRunner : MonoBehaviour
{
	...
	private void Awake()
	{
		ClubPenguin.SceneRefs.SetCelebrationRunner(this);
	...
	public void PlayCelebrationAnimation(List<long> playersToReward)
	{
		foreach (long item in playersToReward)
		{
			...
			if (!dataEntityHandle.IsNull && dataEntityCollection.TryGetComponent(dataEntityHandle, out component) && !(component?.GameObject.IsDestroyed() ?? true))
			{
