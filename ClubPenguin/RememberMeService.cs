...
public class RememberMeService
{
	...
	public int SavedUsernameCount => (Usernames != null) ? Usernames.Count : 0;
	...
	public RememberMeData GetRememberMeData()
	{
		...
		if (!cPDataEntityCollection.TryGetComponent(cPDataEntityCollection.LocalPlayerHandle, out RememberMeData component))
		{
	...
	{
		return new CacheableType<string>($"cp.rememberme.{username}.data", "{}");
	}

	private KeyChainCacheableString getCredentialDataCache(string username)
	{
		return new KeyChainCacheableString($"cp.rememberme.{username}.credential", string.Empty);
	}
