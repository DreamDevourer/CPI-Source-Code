...
public class IglooUIStateController : AbstractIglooUIStateController
{
	...
	public SceneStateData.SceneState CurrentState => sceneStateData.State;

	public SceneStateData.SceneState PreviousState => previousState;
	...
	public void IglooSaveStatusNotification(IglooSaveStatus status, bool showAfterSceneLoad)
	{
		...
		{
			eventDispatcher.DispatchEvent(new IglooUIEvents.ShowNotification(text, 5f, null, adjustRectPositionForNotification: false, showAfterSceneLoad));
		}
	}
	...
	private void onUpdateAndPublishActiveLayout(bool success, SceneLayoutData sceneLayoutData)
	{
		if (success)
		{
			...
			{
				IglooSaveStatusNotification(IglooSaveStatus.ActiveIglooSaveSuccess, showAfterSceneLoad: true);
			}
			else
			{
				IglooSaveStatusNotification(IglooSaveStatus.InActiveIglooSaveSuccess, showAfterSceneLoad: true);
			}
		}
		else
		{
			IglooSaveStatusNotification(IglooSaveStatus.Fail, showAfterSceneLoad: true);
