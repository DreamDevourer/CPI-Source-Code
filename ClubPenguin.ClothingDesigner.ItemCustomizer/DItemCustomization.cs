...
public class DItemCustomization
{
	...
	public Sprite TemplateSprite
	{
		...
		private set
		{
			if (value == null)
			{
				throw new NullReferenceException($"Attempting to set a null value for {TemplateDefinition.AssetName} sprite.");
			}
			templateSprite = value;
		}
	}

	public DCustomizationChannel CurrentChannel => currentChannel;

	public DCustomizationChannel RedChannel => customizationChannelList[0];

	public DCustomizationChannel GreenChannel => customizationChannelList[1];

	public DCustomizationChannel BlueChannel => customizationChannelList[2];
