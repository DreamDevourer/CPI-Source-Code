...
public class InventoryEquipmentModel
{
	...
	public DataEntityHandle LocalPlayerInfo => Service.Get<CPDataEntityCollection>().LocalPlayerHandle;
	...
	{
		long dateTimeCreated = InventoryData.Inventory[equipment2].Data.DateTimeCreated;
		return dateTimeCreated.CompareTo(InventoryData.Inventory[equipment1].Data.DateTimeCreated);
	}
	...
	private void getHiddenItems()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				bool flag2 = true;
				if (dictionary.TryGetValue(InventoryData.Inventory[num].Data.DefinitionId, out TemplateDefinition value) && value.IsEditable)
				{
	...
	private long getEquipmentIdByTemplateName(string templateName)
	{
		...
		throw new NullReferenceException($"No template with the name {templateName} is currently equipped onto the penguin.");
	}
