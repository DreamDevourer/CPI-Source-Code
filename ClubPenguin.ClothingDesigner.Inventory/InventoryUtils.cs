...
public static class InventoryUtils
{
	...
	public static bool IsEquipmentPartEqual(DCustomEquipmentPart part1, CustomEquipmentPart part2)
	{
		...
		for (int i = 0; i < part1.Decals.Length; i++)
		{
			...
			if (Math.Abs(part1.Decals[i].Rotation - part2.customizations[i].rotation) > 1.401298E-45f)
			{
				return false;
			}
			if (Math.Abs(part1.Decals[i].Scale - part2.customizations[i].scale) > 1.401298E-45f)
			{
				return false;
			}
			...
			if (Math.Abs(part1.Decals[i].Uoffset - part2.customizations[i].uoffset) > 1.401298E-45f)
			{
				return false;
			}
			if (Math.Abs(part1.Decals[i].Voffset - part2.customizations[i].voffset) > 1.401298E-45f)
			{
				return false;
			}
		}
