// IglooCatalogPurchaseConfirmation
using ClubPenguin;
...
public class IglooCatalogPurchaseConfirmation : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IEventSystemHandler, IIglooCatalogPurchaseErrorHandler
{
	...
	private void showCoinsStatus()
	{
		if (getCoinAmount() < item.Cost * purchaseCount)
		{
			...
			disabledBuyButton.SetActive(value: true);
		}
		else
		{
			buyButton.SetActive(value: true);
			disabledBuyButton.SetActive(value: false);
		}
	}
	...
	{
		needMoreCoinsTooltip.SetActive(value: true);
		needMoreCoinsTooltip.GetComponent<Animator>().SetBool("IsOpen", value: true);
		yield return new WaitForSeconds(2f);
		needMoreCoinsTooltip.GetComponent<Animator>().SetBool("IsOpen", value: false);
	}
