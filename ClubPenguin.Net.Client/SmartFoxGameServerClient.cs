...
public class SmartFoxGameServerClient : IGameServerClient
{
	...
	public long ServerTime => lastServerTime + clientTimer.ElapsedMilliseconds;
	...
	private IEnumerator processMessagesCoroutine()
	{
		while (true)
		{
			...
			if (mt.TryGetServerTimeUpdate(out long serverTime))
			{
	...
	private void processEvents()
	{
		...
		if (count != 0)
		{
			...
			KeyValuePair<GameServerEvent, object> item = default(KeyValuePair<GameServerEvent, object>);
	...
	private void forceTearDown(object sfsToTeardown)
	{
		...
		mt.teardown();
		if (mt.TryClearSfsRoomToLeave(out Room room))
		{
			if (text == null)
			{
				text = room.Name;
