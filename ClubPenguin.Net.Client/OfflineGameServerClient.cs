...
public class OfflineGameServerClient : IGameServerClient
{
	...
	public class ActionedConsumable : TimedConsumable
	{
		...
		public bool AutoDequipeHeldObject => true;
	...
	public class PartyGameSessionManager
	{
		...
		{
			if (activePartyGames.TryGetValue(sessionId, out IPartyGameSession value))
			{
	...
	public long ServerTime => DateTime.UtcNow.GetTimeInMilliseconds();
	...
	{
		if (eventListeners.TryGetValue(gameServerEvent, out GameServerEventListener value) && value != null)
		{
			try
			{
			...
			{
				Log.LogError(this, "Exception triggered by game server event " + gameServerEvent + ": " + ex.Message);
