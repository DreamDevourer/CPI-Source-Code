...
public class CtsBlockCipher : BufferedBlockCipher
{
	...
	public override int DoFinal(byte[] output, int outOff)
	{
		...
		if (forEncryption)
		{
			cipher.ProcessBlock(base.buf, 0, array, 0);
			if (base.bufOff < num)
			{
		...
		else
		{
			...
			blockCipher2.ProcessBlock(base.buf, 0, array, 0);
			for (int k = num; k != base.bufOff; k++)
			{
				array3[k - num] = (byte)(array[k - num] ^ base.buf[k]);
			}
			Array.Copy(base.buf, num, array, 0, length);
			cipher.ProcessBlock(array, 0, output, outOff);
			Array.Copy(array3, 0, output, outOff + num, length);
		}
