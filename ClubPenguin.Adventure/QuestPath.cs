...
public class QuestPath : MonoBehaviour
{
	...
	private void changeNode(int newNode)
	{
		...
		int pathNodeCountWithinDistance = GetPathNodeCountWithinDistance(currentNode, ForwardDistance, forward: true);
	...
	private void updateNodeEffects()
	{
		...
		int pathNodeCountWithinDistance2 = GetPathNodeCountWithinDistance(currentNode, TrailingDistance, forward: false);
	...
	private int GetPathNodeCountWithinDistance(int startIndex, float distance, bool forward)
	{
		...
		if (forward)
		{
			if (startIndex < nodes.Length)
			{
				...
				{
					QuestPathNode questPathNode = nodes[i];
	...
	{
		string eventName = $"PathNode{nodeIndex}Triggered";
