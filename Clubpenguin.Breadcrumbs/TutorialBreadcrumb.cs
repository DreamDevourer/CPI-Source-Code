...
public class TutorialBreadcrumb : MonoBehaviour
{
	...
	private void OnEnable()
	{
		if (BreadcrumbId != "")
		{
			getController().BreadcrumbIsOnScreen(BreadcrumbId, isOnScreen: true);
			init();
	...
	private void onArrowPrefabLoaded(string path, GameObject prefab)
	{
		if (isShowing && breadcrumbArrow == null)
		{
			...
			breadcrumbArrow.transform.SetParent(base.transform, worldPositionStays: false);
	...
	private void OnDestroy()
	{
		...
		getController().BreadcrumbIsOnScreen(BreadcrumbId, isOnScreen: false);
