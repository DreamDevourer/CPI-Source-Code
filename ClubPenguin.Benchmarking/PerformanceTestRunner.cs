...
public class PerformanceTestRunner : MonoBehaviour
{
	public struct Stats
	{
		...
		{
			return $"[Stats] total: {total}, mean: {mean}, stdDev: {stdDev}, min: {min}, max: {max}, count: {count}";
		}
	}
		...
		try
		{
					...
					for (int i = 0; i < tests.Length; i++)
					{
						...
						try
						{
							...
							for (int frame = 0; frame < frameCount; frame++)
							{
								...
								float[] frameTimes = tests[i].FrameTimes;
								int num;
								index = (num = index) + 1;
								profileLogger.Time("Frame", frameTimes[num]);
							}
			...
			using (ProfileLogger profileLogger3 = new ProfileLogger("Summary"))
			{
				...
				for (int j = 0; j < tests.Length; j++)
				{
					...
					profileLogger3.Print($"Scene: {tests[j].Scene}\nLoad: {Results[j].LoadTime}\nRun: {Results[j].RunTime}\nFrames: {Results[j].FrameTime}\n\n");
				}
			}
		}
		finally
