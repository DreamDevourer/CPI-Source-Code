...
internal class TutorialOverlay : MonoBehaviour
{
	...
	public Vector2 HighlightPosition => highlightPosition;
	...
	private void updatePopup()
	{
		...
		PopupArrow.gameObject.SetActive(value: false);
		if (data.ShowArrow)
		{
	...
	private void UpdateHighlightOutline(bool showOutline)
	{
		if (showOutline)
		{
			if (HighlightOutline == null)
			{
				...
				HighlightOutline.transform.SetParent(HighlightOutlineContainer.transform, worldPositionStays: false);
			}
	...
	private void onArrowAssetLoaded(string path, Sprite sprite)
	{
		...
		PopupArrow.gameObject.SetActive(value: true);
		if (data.ArrowPosition == TutorialOverlayArrowPosition.LEFT || data.ArrowPosition == TutorialOverlayArrowPosition.RIGHT)
		{
			PopupArrow.Find("Icon").GetComponent<Animator>().SetBool("UpDown", value: false);
		}
		else
		{
			PopupArrow.Find("Icon").GetComponent<Animator>().SetBool("UpDown", value: true);
		}
	}
