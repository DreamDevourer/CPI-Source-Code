...
internal class TutorialTooltip : MonoBehaviour
{
	...
	public void Show()
	{
		GetComponent<Animator>().SetBool("IsOpen", value: true);
	}

	public void Hide()
	{
		GetComponent<Animator>().SetBool("IsOpen", value: false);
	}
	...
	public GameObject AddText(string contents, Color color, int fontSize)
	{
		...
		gameObject.transform.SetParent(Bubble.transform, worldPositionStays: false);
	...
	public void SetPosition(Vector2 position)
	{
		...
		float num2 = (float)Screen.width * a.x - GetComponent<RectTransform>().anchoredPosition.x;
		if (x < num)
		{
