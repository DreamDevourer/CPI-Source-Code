...
public class MultiPointLineAttractor : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		Vector3 vector2 = vector * (0.5f - (float)Segments / 2f);
		for (int i = 0; i < Segments; i++)
		{
			oddAttractionPoints[i] = initAttractionPoint(vector2);
			vector2 += vector;
		}
		evenAttractionPoints = new Collider[Segments - 1];
		vector2 = vector * (1f - (float)Segments / 2f);
		for (int i = 0; i < Segments - 1; i++)
		{
			evenAttractionPoints[i] = initAttractionPoint(vector2);
			vector2 += vector;
		}
	}
