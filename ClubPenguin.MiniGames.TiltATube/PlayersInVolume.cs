...
public class PlayersInVolume : MonoBehaviour
{
	...
	{
		if (dataEntityCollection.TryGetComponent(playerHandle, out DisplayNameData component))
		{
			return component.DisplayName;
		}
		return "Error: name not found";
	...
	private string getPlayerName(RemotePlayerData remotePlayerData)
	{
		...
		if (dataEntityCollection.TryGetComponent(entityByComponent, out DisplayNameData component))
		{
			return component.DisplayName;
		}
		return "Error: name not found";
	...
	private void onPlayerDisconnect(RemotePlayerData remotePlayerData)
	{
		if (deleteRemotePlayer(remotePlayerData))
		{
			...
			if (Service.Get<CPDataEntityCollection>().TryGetComponent(entityByComponent, out GameObjectReferenceData component))
			{
