...
public class MixWebCallFactory : IMixWebCallFactory, IDisposable
{
	...
	private IWebCall<TRequest, TResponse> CreateWebCall<TRequest, TResponse>(HttpMethod method, Uri uri, TRequest request, long latencyWwwCallTimeout, long maxWwwCallTimeout) where TRequest : BaseUserRequest where TResponse : BaseResponse, new()
	{
		...
		MixWebCall<TRequest, TResponse> webCall = (MixWebCall<TRequest, TResponse>)new MixWebCall<TRequest, TResponse>(logger, uri, body, method, dictionary2, wwwCallFactory, webCallEncryptor, latencyWwwCallTimeout, maxWwwCallTimeout, database, swid);
		webCalls.Add(webCall);
		webCallQueue.AddWebCall((IWebCall<TRequest, TResponse>)webCall);
		if (isSessionRefreshing)
		{
			((MixWebCall<TRequest, TResponse>)webCall).RefreshStatus = WebCallRefreshStatus.WaitingForRefreshCallback;
		}
		((MixWebCall<TRequest, TResponse>)webCall).OnResponse += delegate
		{
			webCalls.Remove(webCall);
			webCallQueue.RemoveWebCall((IWebCall<TRequest, TResponse>)webCall);
		};
		((MixWebCall<TRequest, TResponse>)webCall).OnError += delegate
		{
			webCalls.Remove(webCall);
			webCallQueue.RemoveWebCall((IWebCall<TRequest, TResponse>)webCall);
		};
		((MixWebCall<TRequest, TResponse>)webCall).OnUnauthorized += delegate(object sender, WebCallUnauthorizedEventArgs e)
		{
			HandleOnUnauthorized((IWebCall<TRequest, TResponse>)webCall, e);
		};
		return (IWebCall<TRequest, TResponse>)webCall;
	}
