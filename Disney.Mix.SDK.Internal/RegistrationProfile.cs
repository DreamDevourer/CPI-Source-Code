...
public class RegistrationProfile : IInternalRegistrationProfile, IRegistrationProfile
{
	...
	private void UpdateDisplayName(Disney.Mix.SDK.Internal.GuestControllerDomain.DisplayName displayName)
	{
		if (displayName != null)
		{
			if (!DateTime.TryParse(displayName.moderatedStatusDate, out DateTime result))
			{
	...
	{
		MarketingItems = marketing?.Select((Disney.Mix.SDK.Internal.GuestControllerDomain.MarketingItem p) => new KeyValuePair<string, bool>(p.code, p.subscribed));
	}
